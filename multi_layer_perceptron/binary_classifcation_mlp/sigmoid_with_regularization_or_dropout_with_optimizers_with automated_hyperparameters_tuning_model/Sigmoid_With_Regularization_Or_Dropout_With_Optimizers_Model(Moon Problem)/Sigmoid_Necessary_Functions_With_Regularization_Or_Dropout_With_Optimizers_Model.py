...
def random_layers_dims(X_train):
...
def automated_hyperparameters_tuning(X_train, Y_train):
...
def automated_hyperparameters_tuning_regularized_version(X_train, Y_train):
    
    train_accuracy = 0
    test_accuracy = 0
    lst_param_train_test_accuracy = []
    while(train_accuracy < 0.99 and test_accuracy < 0.98):
        layers_dims = random_layers_dims(X_train)
        minibatch_size = random_minibatch_size(X_train)
        lambd = random_lambd()
        optimizer = random_optimizer()
        learning_rate = random_alpha()
        parameters = regularized_or_dropout_optimizers_model_sigmoid(X_train, Y_train, layers_dims = layers_dims, minibatch_size = minibatch_size, lambd = lambd, keep_prob = 0, optimizer = optimizer,  num_epochs = 10000, learning_rate = learning_rate, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8)
        train_accuracy = predict_accuracy(X_train, Y_train, parameters)
        test_accuracy = predict_accuracy(X_test, Y_test, parameters)

        
        tpl_param_accuracy = (parameters, train_accuracy, test_accuracy)
        lst_param_train_test_accuracy.append(tpl_param_accuracy)
        parameters_for_max_train_accuracy, max_train_accuracy, max_test_accuracy = max(lst_param_train_test_accuracy,key=lambda item:item[2])
...
def automated_hyperparameters_tuning_dropout_version(X_train, Y_train):
    ...
    while(train_accuracy < 0.99 and test_accuracy < 0.98):
        ...
        optimizer = random_optimizer()
        learning_rate = random_alpha()
        parameters = regularized_or_dropout_optimizers_model_sigmoid(X_train, Y_train, layers_dims = layers_dims, minibatch_size = minibatch_size, lambd = 0, keep_prob = keep_prob, optimizer = optimizer,  num_epochs = 10000, learning_rate = learning_rate, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8)
        ...
        hyperparameters_for_max_train_accuracy = (layers_dims, minibatch_size, keep_prob, optimizer, learning_rate)
