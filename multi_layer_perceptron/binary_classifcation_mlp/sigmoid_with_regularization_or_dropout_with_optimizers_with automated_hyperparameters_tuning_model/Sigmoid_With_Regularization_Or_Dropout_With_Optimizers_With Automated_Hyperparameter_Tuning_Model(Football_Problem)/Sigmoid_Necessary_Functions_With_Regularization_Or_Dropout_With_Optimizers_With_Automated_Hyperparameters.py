...
# this definition allows only to use regularized or dropout or none at a time although it is possible to use both at the same time.
...
def automated_hyperparameters_tuning_no_regularization_no_dropout(X_train, Y_train, X_test, Y_test):
    ...
    while(test_accuracy < 0.94):
        ...
        parameters = regularized_or_dropout_optimizers_model_sigmoid(X_train, Y_train, layers_dims = layers_dims, minibatch_size = minibatch_size, lambd = 0, keep_prob = 0, optimizer = optimizer,  num_epochs = 1000, learning_rate = learning_rate, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8)
        tac = time.time()
...
def automated_hyperparameters_tuning_dropout_version(X_train, Y_train, X_test, Y_test):
    ...
    while(test_accuracy < 0.94):
        ...
        learning_rate = random_alpha()
        tic = time.time()
        parameters = regularized_or_dropout_optimizers_model_sigmoid(X_train, Y_train, layers_dims = layers_dims, minibatch_size = minibatch_size, lambd = 0, keep_prob = keep_prob, optimizer = optimizer,  num_epochs = 1000, learning_rate = learning_rate, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8)
        tac = time.time()
        training_time = tac-tic
